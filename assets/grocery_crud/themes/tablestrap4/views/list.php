<table cellpadding="0" cellspacing="0" border="0" class="display groceryCrudTable table table-striped" id="<?php echo uniqid(); ?>">
	...
	<tbody>
		<?php foreach ($list as $num_row => $row) { ?>
			<tr id='row-<?php echo $num_row ?>'>
				...
				<?php } ?>
				<?php if (!$unset_delete || !$unset_edit || !$unset_read || !empty($actions)) { ?>
					<td class='actions' style="display: flex; justify-content: flex-end; flex-flow: column-wrap; flex-wrap: wrap;">
						...
						<?php if (!$unset_delete) { ?>
							<a onclick="javascript: return delete_row('<?php echo $row->delete_url ?>', '<?php echo $num_row ?>')" href="javascript:void(0)" class="delete_button btn btn-xs btn-danger btn-sm hidden-xs mb-1 mr-1" role="button">
	...
	<tfoot>
		<tr>
			...
			<?php if (!$unset_delete || !$unset_edit || !$unset_read || !empty($actions)) { ?>
				<th>
					<button class="btn btn-primary refresh-data btn-block" role="button" data-url="<?php echo $ajax_list_url; ?>">
						<i class="fas fa-sync-alt"></i> <?php echo $this->l('list_clear_filtering'); ?>
					</button>
				</th>
			<?php } ?>
		</tr>
